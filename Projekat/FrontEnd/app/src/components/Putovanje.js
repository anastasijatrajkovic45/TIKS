...
const handleRezervacijaPutovanja = async (putovanjeId) => {
  setRezervacijaAktivna(true); // Otvori formu za rezervaciju
  // Postavite putovanje za koje se vrši rezervacija
...
const handleRezervacija = async () => {
  try {
    ...
    if (response.ok) {
      console.log('Rezervacija je uspješno dodana.');
      setRezervacijaAktivna(false); // Zatvori formu za rezervaciju
      // Osvježi podatke na stranici nakon uspješne rezervacije
